$j(function() {
	...
	var buildEventForm = function() {
		...
		form
			...
			.add({
				...
				emptyFirstOption: false,
				required: true,
				help: 'Select the color class that would be used to diplay this event in calendars.'
			})
			.addHtml('<div class="clearfix"></div>')
	...
	var displayNewCalendarForm = function(show) {
		...
		if(show) {
			...
			var options = eventIds.map(function(e) {	
				return { 
					id: e, 
					text: '<span class="label label-' + _data.events[e].color + '">' +
	...
	var populateEventForm = function(type) {
		var form = AppGiniPlugin.form('#event-form'),
			ev = getEvent(type),
			vars = ['color','table','customWhere','title','allDay','startDateField','startTimeField','endDateField','endTimeField'];
